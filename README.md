...
6. When sender authentication is required, the message must have a DMARC `pass` verdict. Mail that
   does not pass is escalated without calling Claude or creating a draft.
7. The message is converted to plain text. Control characters, instruction-like lines, and
   unapproved URLs are removed.
8. Claude classifies the message as `needs_reply`, `ignore`, or `escalate`.
9. Routine replies are saved with `createInboxReplyDraft` and the current `reply_version`.
10. A draft is sent only when `AUTO_SEND=true`. Draft creation and sending use separate stable
...
- DMARC enforcement before model or draft creation when `REQUIRE_AUTHENTICATED_SENDER=true`
...
Shipmail returns SPF, DKIM, and DMARC verdicts for each inbox message. This starter requires DMARC
to pass when sender authentication enforcement is on. DMARC ties the visible `From` domain to an
aligned SPF or DKIM result. SPF alone is not enough because it checks the envelope sender, which
can differ from the visible `From` address.

If the authentication results are missing, malformed, null, or DMARC has any verdict other than
`pass`, the message stays in the reply queue for a person to review. The verdicts and reason are
logged. Claude is not called and no draft is created. Enforcement defaults to on with `AUTO_SEND`
and off in draft-only mode. Set `REQUIRE_AUTHENTICATED_SENDER` explicitly to override that default.

Allowlisting reduces exposure but does not make a sender trustworthy when authentication
enforcement is off. Model defenses are not a proof against every future prompt-injection technique.
